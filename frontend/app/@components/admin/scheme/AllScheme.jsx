...
const AllCourses = () => {
  const { isLoading, data, refetch } = useGetSchemesQuery(
    {},
    { refetchOnMountOrArgChange: true }
  );
  const [deleteScheme, { isSuccess, error }] = useDeleteSchemeMutation();
  ...
  const columns = [
    ...
    { field: "level", headerName: "Type", flex: 0.2 },
    { field: "ministry", headerName: "Ministry", flex: 0.6 },
    { field: "updated_at", headerName: "Last Updated", flex: 0.4 },
    {
      ...
      flex: 0.2,
      renderCell: (params) => (
        <Link href={`/admin/edit-scheme/${params.row.id}`}>
...

  {
    data &&
      data.schemes.forEach((item) => {
        rows.push({
          id: item._id,
          title: item.basicDetails.schemeName,
          level: item.basicDetails.level,
          ministry: item.basicDetails.ministryName,
